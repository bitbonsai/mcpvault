...
export class FileSystemService {
   ...
   */
  async backupToTrash(relativePath: string, snapshotContent?: string): Promise<string> {
   ...
   */
  async softDeleteNote(path: string): Promise<{ success: boolean; path: string; trashPath?: string; message: string }> {
    if (!this.pathFilter.isAllowed(path)) {
      return {
        success: false,
        path,
