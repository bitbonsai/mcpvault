...
describe("append-only overwrite protection", () => {
  ...
  test("refuses overwrite of a configured append-only file", async () => {
    ...
    expect((await fs.readNote("log.md")).content).toContain("entry 1");
  });

  test("matches on basename, in any folder", async () => {
    const fs = new FileSystemService(vault, undefined, undefined, ["log.md"]);
    await fs.writeNote({ path: "nested/log.md", content: "a\n", mode: "append" });
    await expect(
      fs.writeNote({ path: "nested/log.md", content: "wiped\n", mode: "overwrite" })
    ).rejects.toThrow(/append-only/i);
  });
