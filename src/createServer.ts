...
export function createServer(vaultPath: string, options: CreateServerOptions = {}): { server: Server; index: VaultIndex } {
  const {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (toolName) {
        ...
        case "suggest_links": {
          const suggestions = vaultIndex.suggestLinks(
            trimmedArgs.path,
            trimmedArgs.limit || 10
          );
          const indent = trimmedArgs.prettyPrint ? 2 : undefined;
          return {
            content: [{ type: "text", text: JSON.stringify({
